...
//	email  : pinotoy@yahoo.com
	...
	var imgDir = '<cfoutput>#imgpath#/</cfoutput>';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	...
	function popUpCalendar(ctl, ctl2, format) {
		...
		if (bPageLoaded) {
			if (crossobj.visibility == 'hidden') {
				...
				crossobj.left = (fixedX == -1) ? ctl.offsetLeft + leftpos : (fixedX == -2) ? ctl.offsetLeft + leftpos + ctl.offsetWidth + 2 : fixedX;
				crossobj.top = (fixedY == -1) ? ctl.offsetTop + toppos + ctl.offsetHeight + 2 : (fixedY == -2) ? ctl.offsetTop + toppos : fixedY;
