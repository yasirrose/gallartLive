...
<!--- The reCAPTCHA response is verified during the checkout-to-review handoff. --->
<cfif NOT len(checkoutError)>
...
<!--- Consume the token under a session lock so only one request can create an order. --->
<cfif NOT len(checkoutError)>
...
<!--- Get contents of the validated cart and tracker session. --->
